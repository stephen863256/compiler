...
def evaluate(test_base_path, testcases):
    ...
    for index, case in enumerate(testcases):
        ...
        EXE_GEN_PTN = ["loongarch64-unknown-linux-gnu-gcc", "-static" ,asm_path, LIB_PATH ,"-o", exe_path]
        EXE_PTN = ["qemu-loongarch64", exe_path]
                    
        asmbuilder_result = subprocess.run(ASM_GEN_PTN, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
